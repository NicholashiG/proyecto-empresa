   ...
   // private HashMap<String, String> mapaPaises;
    ...
    public void reservaHotelShowTableData() throws Exception{
        ...
        for (Object[] result : results) {
            ReservaHotelData data = new ReservaHotelData(
                    ...
                    (Integer) result[8], // cantidad_habitaciones
                    (String) result[9],  // estado
                    (String) result[10],  // descripcion
                    String.valueOf(result[11])  // id_habitacion
            );
    ...
    private void crearReservaHotel() {
            ...
            try {
                ...
                empleadoLogin, hotelSeleccionado, regimenSeleccionado, politicaCancelacionSeleccionada);
    ...
    private void actualizarReservaHotel() {
        ...
        try {
            ...
            reservaSeleccionada.setPoliticaCancelacion(politicaCancelacionSeleccionada);
    ...
    void comboHotelesListener(ActionEvent event) {
        ...
        if(gr_checkin_date.getValue() != null){
            cargarPlazoCancelacion(idHotel);
        }
    }

    void cargarPlazoCancelacion(String idHotel) {
        try {
            String plazoCancelacion = reservaHotelRepo.obtenerPlazoCancelacion(idHotel);
            LocalDate fechaCheckin = gr_checkin_date.getValue();
            int plazoCancelacionDias = Integer.parseInt(plazoCancelacion);
            LocalDate fechaLimiteCancelacion = fechaCheckin.minusDays(plazoCancelacionDias);
            if (fechaLimiteCancelacion.isBefore(LocalDate.now())) {
                gr_plazocancelar_lbl.setText("Fecha límite alcanzada");
                gr_plazocancelar_lbl.setTextFill(Color.RED);
            } else {
                gr_plazocancelar_lbl.setText(fechaLimiteCancelacion.toString());
                gr_plazocancelar_lbl.setTextFill(Color.BLACK);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
